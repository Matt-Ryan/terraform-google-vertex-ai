...
- **Reading per-gateway values.** The module exposes the whole engine as the `policy_engine` output. The per-gateway computed values (`dns_record`, `ip_address`, `psc_endpoint`, `state`) live inside `policy_engine.gateway_configs`, which is a **set** and so cannot be indexed by gateway name directly. Re-key it into a map to look one up — `{ for g in module.<NAME>.policy_engine.gateway_configs : g.name => g.dns_record }` — then index by name (e.g. `...["my-gateway"]`). The [gateway example](../../examples/semantic-governance-policy-engine-gateway-example) shows this in its `outputs.tf`.
